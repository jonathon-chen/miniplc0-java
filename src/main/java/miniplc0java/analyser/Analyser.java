...
public final class Analyser {
    ...
    private void analyseStatementSequence() throws CompileError {
        ...
        while (true) {
            ...
            if (peeked.getTokenType() == TokenType.Ident) {
            	analyseAssignmentStatement();// 调用相应的分析函数
    ...
    private void analyseAssignmentStatement() throws CompileError {
        ...
        if (symbol == null) {
            // 没有这个标识符
            throw new AnalyzeError(ErrorCode.NotDeclared, tokenname.getStartPos());
    ...
    private void analyseFactor() throws CompileError {
        ...
        if (check(TokenType.Ident)) {
            ...
            if (symbol == null) {
                ...
                // 标识符没初始化
                throw new AnalyzeError(ErrorCode.NotInitialized, tokenname.getStartPos());
            }
            var offset = getOffset(name, null);
            instructions.add(new Instruction(Operation.LOD, offset));
        } else if (check(TokenType.Uint)) {
        	Token tokenname=next();// 是整数
            // 加载整数值
            int value = (int)tokenname.getValue();
            instructions.add(new Instruction(Operation.LIT, value));
